...
**And** the request includes session identification (see AC-10)
...
**And** the badge is visible in the desktop navigation, and in the mobile navigation once the shopper opens the navigation menu
...
### AC-10: Session identification

**Given** a shopper interacts with the cart
**When** any cart API request is made
**Then** the session is identified by the `X-Session-ID` request header
**And** if the header is not provided, the session defaults to `"workshop-demo"`, also when a `session_id` cookie is sent
**And** on the shop's pages, the shopper's cart is identified by the `session_id` cookie
...
|---------|-----------|------------------|-------------|--------|
| Unreleased | WEB-005_AC-1 | Given a shopper is viewing a product card on any page (home, products, detail)<br>When the shopper clicks the "Add to Cart" button<br>Then a POST request is sent to `/api/cart/items` with the product ID<br>And the request includes session identification (see AC-8) | Given a shopper is viewing a product card on any page (home, products, detail)<br>When the shopper clicks the "Add to Cart" button<br>Then a POST request is sent to `/api/cart/items` with the product ID<br>And the request includes session identification (see AC-10) | Updated the cross-reference to the session identification criterion, which is now AC-10; the request is unchanged. |
| Unreleased | WEB-005_AC-2 | Given a shopper has clicked "Add to Cart" on a product<br>When the API responds successfully<br>Then the cart count badge in the navigation header updates to reflect the new total item count<br>And the badge is visible in both desktop and mobile navigation | Given a shopper has clicked "Add to Cart" on a product<br>When the API responds successfully<br>Then the cart count badge in the navigation header updates to reflect the new total item count<br>And the badge is visible in the desktop navigation, and in the mobile navigation once the shopper opens the navigation menu | Clarified where shoppers on small screens find the badge: the mobile navigation sits behind the navigation menu button, and the badge is shown there once the menu is open. |
| Unreleased | WEB-005_AC-10 | Given a shopper interacts with the cart<br>When any cart API request is made<br>Then the session is identified by one of:<br>- `X-Session-ID` request header<br>- `session_id` cookie<br>And if neither is provided, the session defaults to `"workshop-demo"` | Given a shopper interacts with the cart<br>When any cart API request is made<br>Then the session is identified by the `X-Session-ID` request header<br>And if the header is not provided, the session defaults to `"workshop-demo"`, also when a `session_id` cookie is sent<br>And on the shop's pages, the shopper's cart is identified by the `session_id` cookie | Replaces WEB-005_AC-8: API clients identify their cart with the `X-Session-ID` header, and the `session_id` cookie identifies the shopper's cart on the shop's pages. |
