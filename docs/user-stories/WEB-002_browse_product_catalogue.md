...
**And** each product card shows name, image, price (the product's price), and an "Add to Cart" button

### AC-2: Category filter checkboxes

**Given** the products page has loaded
**When** the shopper views the filter sidebar/section
**Then** a "Categories" filter group is displayed with checkboxes
...
|---------|-----------|------------------|-------------|--------|
| Unreleased | WEB-002_AC-1 | Given a shopper navigates to the products page (`/products`)<br>When the page finishes loading<br>Then all 12 seed products are displayed in a grid layout<br>And each product card shows name, image, price, and an "Add to Cart" button | Given a shopper navigates to the products page (`/products`)<br>When the page finishes loading<br>Then all 12 seed products are displayed in a grid layout<br>And each product card shows name, image, price (the product's price), and an "Add to Cart" button | Clarified that the price shown on each product card is the product's price. |
| Unreleased | WEB-002_AC-2 | Given the products page has loaded<br>When the shopper views the filter sidebar/section<br>Then a "Category" filter group is displayed with checkboxes<br>And checkboxes are available for all 9 categories present in the seed data<br>And no category checkbox is pre-selected by default | Given the products page has loaded<br>When the shopper views the filter sidebar/section<br>Then a "Categories" filter group is displayed with checkboxes<br>And checkboxes are available for all 9 categories present in the seed data<br>And no category checkbox is pre-selected by default | Clarified the heading of the category filter group, which shoppers see as "Categories"; the group and its checkboxes are unchanged. |
