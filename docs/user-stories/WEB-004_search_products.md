...
**When** the shopper clicks the "Clear search" or "Clear results" button
...
**Given** an API consumer searches the catalogue
**When** the consumer sends `GET /api/search/` with the search text in the `query` parameter
**Then** the response is successful
**And** it lists the products that match the search text
...
|---------|-----------|------------------|-------------|--------|
| Unreleased | WEB-004_AC-6 | Given search results are currently displayed on the page<br>When the shopper clicks the "Clear search/results" button<br>Then the search results section is hidden<br>And the normal page content (product grid or hero section) is shown again<br>And the search input is cleared | Given search results are currently displayed on the page<br>When the shopper clicks the "Clear search" or "Clear results" button<br>Then the search results section is hidden<br>And the normal page content (product grid or hero section) is shown again<br>And the search input is cleared | Clarified the button's label: shoppers see "Clear search" on the products page and "Clear results" on the home page; clearing works the same on both. |
| Unreleased | WEB-004_AC-9 | Given a shopper submits a search query<br>When the search request is processed<br>Then the frontend makes a call to the `/api/search` endpoint with the query parameter<br>And the API returns matching products in the response | Given an API consumer searches the catalogue<br>When the consumer sends `GET /api/search/` with the search text in the `query` parameter<br>Then the response is successful<br>And it lists the products that match the search text | Replaces WEB-004_AC-4, which described how the page requests its search results: the pages now load their results from the shop itself, so the search API is described from the API consumer's side, with the `query` parameter it reads. |
